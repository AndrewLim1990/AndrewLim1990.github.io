...
1. **$\epsilon$ decay rate was too fast**: Perhaps the neural network did not have enough time/epochs to learn that taking the `EndTurn` action would be beneficial during higher exploration phase. I decreased the decay rate by a factor of 100. i.e. I increased the number of actions to go from 90% to 10% exploration from 50,000 actions to 5,000,000 actions. This seemed to help quite a lot.
